    ...
    <div id="page-content-wrapper">
      ...
      <div class="container-fluid">
        ...
        <div class="text-secondary col-11" style=" background-color: white; box-shadow: 0 3px 5px rgba(0,0,0,0.5); padding: 10px 20px; margin:auto; border-radius: 3px;">
          <b><i class="oi oi-dollar"></i>IPD Cash Billing</b>
        </div> 
          <div class="page_scroller">
            <div class="form-row">
              <div class="form-group col-sm-12 col-md-2">
                <select class="form-control form-control-sm" id="searchBy">
                  ...
                  <option value="refno">IPD No.</option>
                </select>
              </div>
            ...
            <table class="table table-sm table-striped table-bordered">
              ...
              <?php 
                include('../ConnectionClass.php');
                $sql = "SELECT * FROM tbl_ipd_admission WHERE adm_no IN(SELECT fileno FROM tbl_ipd_service_request WHERE req_status != 'cleared' AND payment_type='Cash')";
                $rows = $db->ReadArray($sql);
                foreach($rows as $row):
                  $patient = $db->ReadOne("SELECT * FROM tbl_patient WHERE refno='$row[refno]'");
                  $bill_amount = 0;
                  $bills = $db->ReadArray("SELECT req_cost from tbl_ipd_service_request WHERE fileno='$row[adm_no]' AND payment_type='Cash'");
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      <button class="btn btn-sm btn-outline-success" onclick="RecordipdCashBillPaymentAndClearBill('<?= $row['adm_no']?>')"><i class="oi oi-dollar"></i>  Receive Payment</button>
                    </td>
                  </tr>
                <?php
                endforeach;
...
<script type="text/javascript">
  ...
  function ViewBill(fileno,td){
    ...
    RichUrl($('#bills_list'),{GetIPDCashBillsList:'1',fileno:fileno});
  }
